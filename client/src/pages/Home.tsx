...
import PartyFormation from "@/components/variants/PartyFormation";
import ProgressionStepper from "@/components/variants/ProgressionStepper";
import TopPortal from "@/components/variants/TopPortal";
import { StatGauge, UpgradePriorities } from "@/components/variants/StatGauge";
...
type EquipmentAction = { recommendationKey: string; statLabel: string; action: "主ステータスを変更" | "サブステータスを厳選"; slot: string; slotScope?: "specific" | "any"; equippedName: string | null; currentMain: string | null; desiredStat: string; reason: string };
...
export default function Home() {
  ...
  const gaugeVariant = useDisplayVariant("statGauge");
  const formationVariant = useDisplayVariant("partyFormation");
  const portalVariant = useDisplayVariant("topPortal");
  ...
  // 部位を特定しない提案は、部位ラベルも説明も言語別に出す（日本語の「全部位」を他言語の画面へ出さない）。
  const actionSlotLabel = (action: EquipmentAction) => action.slotScope === "any" ? t("anySlot") : action.slot;
  const actionReason = (action: EquipmentAction) => {
    if (action.slotScope === "any") return t("anySlotSubstatReason").replace("{stat}", action.desiredStat);
  ...
  // 案 A の強化提案用。現行カードと同じ文言を返す（表示の分岐だけで、算出は変えない）。
  const recommendationRationale = (item: PriorityRecommendation) => language === "ja" ? item.rationale : `${t("target")} ${item.target}${item.unit} / -${item.deficit.toFixed(item.unit === "%" ? 1 : 0)}${item.unit}`;
      ...
      {selected && (
        <section className="mt-10">
          <div className="grid gap-7 lg:grid-cols-[.78fr_1.22fr]">
            <aside data-surface="dark" className="relative min-h-[390px] overflow-hidden bg-stone-900 p-6 text-stone-50 sm:p-8">
