...
describe("推奨パーティー編成カタログ", () => {
  ...
  it("全キャラクターへ個別の最大3案、選択キャラクター、SNS補助根拠を登録する", () => {
    ...
    allCharacters.forEach(({ game, name }) => {
      ...
      expect(options.every((option) => option.communitySources.some((source) => /^2026-(08-(25|26|27)|09-(07|08|11|13|24))$/.test(source.checkedAt) && source.url.startsWith("https://")))).toBe(true);
    });
  });
