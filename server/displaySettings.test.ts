...
describe("T1 表示バリアントの解決規則", () => {
  it("取得成功時は公開スナップショットだけを使い、欠落は legacy（過去キャッシュへ戻らない）", () => {
    ...
    expect(resolveDisplayVariants({ published: { status: "success", variants: {} }, cached, preview: null, isAdmin: false }))
      .toEqual({ progressionStepper: "legacy", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
    expect(resolveDisplayVariants({ published: { status: "success", variants: { progressionStepper: "stepper" } }, cached: null, preview: null, isAdmin: false }))
      .toEqual({ progressionStepper: "stepper", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
  });

  it("取得中は期限内キャッシュ、取得失敗は全キー legacy", () => {
    const cached = { progressionStepper: "stepper" };
    expect(resolveDisplayVariants({ published: { status: "loading" }, cached, preview: null, isAdmin: false })).toEqual({ progressionStepper: "stepper", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
    expect(resolveDisplayVariants({ published: { status: "loading" }, cached: null, preview: null, isAdmin: false })).toEqual({ progressionStepper: "legacy", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
    expect(resolveDisplayVariants({ published: { status: "error" }, cached, preview: null, isAdmin: false })).toEqual({ progressionStepper: "legacy", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
  });

  it("プレビューは管理者だけに適用し、取得失敗中も維持する", () => {
    const preview = { progressionStepper: "stepper" };
    expect(resolveDisplayVariants({ published: { status: "success", variants: {} }, cached: null, preview, isAdmin: false })).toEqual({ progressionStepper: "legacy", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
    expect(resolveDisplayVariants({ published: { status: "success", variants: {} }, cached: null, preview, isAdmin: true })).toEqual({ progressionStepper: "stepper", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
    expect(resolveDisplayVariants({ published: { status: "error" }, cached: null, preview, isAdmin: true })).toEqual({ progressionStepper: "stepper", statGauge: "legacy", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
  });
  ...
  it("R2 キーごとに独立して解決し、別キーのバリアント名は受け付けない", () => {
    expect(resolveDisplayVariants({ published: { status: "success", variants: { statGauge: "gauge" } }, cached: null, preview: { progressionStepper: "stepper" }, isAdmin: true }))
      .toEqual({ progressionStepper: "stepper", statGauge: "gauge", partyFormation: "legacy", topPortal: "legacy", siteChrome: "legacy" });
