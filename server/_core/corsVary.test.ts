...
/**
 * CORS の `Vary: Origin` の回帰ゲート（設計: docs/修正設計書_tRPC応答のVary_Origin欠落_Phase41_2026-09-15.md D3）。
 * tRPC は応答ヘッダーの既定の `vary`（11.18.0 では `trpc-accept, accept`）を setHeader で書き、先に CORS が付けた `Vary: Origin` を消す。
