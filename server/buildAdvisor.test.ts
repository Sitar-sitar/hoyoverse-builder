  ...
  it("収集済みの全キャラクター一覧に個別プロファイル・基準日・参照範囲を割り当てる", () => {
    ...
    (Object.keys(CHARACTER_GUIDE_METADATA) as Array<keyof typeof CHARACTER_GUIDE_METADATA>).forEach((game) => {
      ...
      CHARACTER_GUIDE_CATALOG[game].forEach((name) => {
        expect(CHARACTER_GUIDE_METADATA[game][name]).toMatchObject({ profileId: expect.any(String), dataAsOf: expect.stringMatching(/^2026-(05-31|06-05|07-31|08-(11|13|14|18|19|21|24|25|26|27)|09-(01|05|07|08|11|12|24))$/), updatedAt: expect.stringMatching(/^2026-(08-(18|25|26|27)|09-(05|07|08|11|13|24))$/) });
      });
    });
